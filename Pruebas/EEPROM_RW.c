...
#include "eeprom.h"                     // Libreria de la memoria eeprom interna
...
void main()
{
    ...
    Uart_Send_String("Escribiendo datos...\r\n");
    for(char i=0; i<10; i++)
    {
        EEPROM_Write(i, i+1);
    }
    ...
    while(1)
    {
        for(char i=0; i<10; i++)
        {
            unsigned char dato = EEPROM_Read(i);
            sprintf(buffer, "Dato: %u , Direccion: %u\r\n", dato, i);
